...
B2BB2G V2는 아래 17개 Engine으로 구성한다.
...
2. **Approval Engine**, **Admin Control Engine**, **Analytics Engine**은 Cross-cutting Engine으로, 모든 도메인 Engine이 이들에 의존할 수 있지만 이들은 도메인 Engine의 세부 구현에 의존하지 않는다.
...
이 5개 없이는 어떤 도메인 Engine도 의미 있게 동작하지 않는다.
...
> Phase 순서는 의존 관계(5장)를 그대로 반영한다. Phase 2(Trade Brokerage)를 Phase 1보다 먼저 시작하지 않는 이유는, Trade Brokerage Engine이 Marketplace/Buy Request Engine의 출력을 소비하기 때문이다.
