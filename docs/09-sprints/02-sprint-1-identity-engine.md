...
| Task 03 - Auth Role Resolver Wiring | Complete | Wired account-role-first effective role resolution into auth guards and public session context while preserving legacy `member_type_id` fallback and existing route API shape. |
| Task 04 - Role Application Flow | Complete | Stabilized role application request/read/cancel server flow: duplicate submitted/reviewing requests now fail clearly, owner-only cancellation uses `cancelled`, and admin approval/rejection remains deferred. |
| Task 05 - Admin Role Approval Preparation | Complete | Added admin-only pending role application query plus guarded approve/reject server actions that use server client/RLS, update `account_roles` and `role_applications`, and leave audit integration as TODO. |
