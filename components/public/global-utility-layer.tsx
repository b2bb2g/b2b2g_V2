    ...
    <>
      <button
        ...
        className={`fixed right-5 z-[80] inline-flex h-11 w-11 items-center justify-center rounded-full border border-slate-200/80 bg-white/90 text-[20px] font-bold text-slate-900 shadow-[0_18px_46px_rgb(43_67_104_/_18%)] backdrop-blur-xl transition duration-200 hover:text-action-blue ${
          isConsentVisible ? "bottom-[112px] sm:bottom-[104px]" : "bottom-5"
      ...
      {isConsentVisible ? (
        <section
          className="fixed inset-x-3 bottom-3 z-[70] mx-auto grid max-w-[360px] gap-2 rounded-[18px] border border-slate-200/80 bg-white/95 p-2.5 shadow-[0_16px_44px_rgb(43_67_104_/_16%)] backdrop-blur-xl sm:inset-x-auto sm:right-5 sm:mx-0 sm:w-[360px]"
          aria-label={t("cookie.title")}
        >
          <div className="min-w-0">
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-[10px] font-extrabold leading-tight text-action-blue">{t("cookie.eyebrow")}</p>
                <h2 className="mt-0.5 text-[14px] font-bold leading-tight text-calm-ink">{t("cookie.title")}</h2>
              </div>
              <button
            ...
            <p className="mt-1.5 hidden text-[11px] leading-5 text-calm-ink-muted-80 sm:block">{t("cookie.description")}</p>
            {cookiePanelOpen ? (
          ...
          <div className="grid grid-cols-2 gap-1.5">
            <button
              className="inline-flex min-h-8 items-center justify-center rounded-full bg-action-blue px-3 text-[11px] font-bold text-white transition hover:bg-action-blue-focus"
            ...
            <button
              className="inline-flex min-h-8 items-center justify-center rounded-full border border-action-blue/20 bg-white px-3 text-[11px] font-bold text-action-blue transition hover:bg-action-blue/5"
              onClick={() => saveConsent(createConsent({ analytics: false, functional: false }))}
              type="button"
            >
            ...
            <button
              className="col-span-2 text-center text-[11px] font-bold text-calm-ink-muted-80 transition hover:text-action-blue"
